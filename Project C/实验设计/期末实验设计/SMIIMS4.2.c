...
#define F_PATH "C://Users/Administrator/Desktop/Item_Information.txt"
    ...
    switch (operation) {
      ...
      NAME: scanf("%s", name);
        ...
        case '3':
            ...
            scanf("%s", category);
            strcpy(items[i] -> item_cate, category);
            break;
        case '4':
...
void print (void)
{
    ...
    for (i = 0; i < j; i++){
        ...
        printf("%-15s\t",items[i]->item_name);
        printf("%-15s\t",items[i]->item_cate);
...
SORT:printf("Use ID/NAME/PRICE/STOCK to sort <I/N/P/S>: ");
    ...
    fp = fopen(F_PATH, "w");
    fputs("ID	NAME		CATEGORY	PRICE		STOCK\n",fp);
...
EDIT:printf("Use ID/NAME to search the item <I/N>: ");
    ...
    switch (search_way){
        ...
        case 'I' :
            ...
            for (i = 0; i < j; i++){
                if (items[i]->item_id == id){
                    printf("The item you want to edit below: \n");
        			printf("\nID\t\tNAME\t\tCATEGORY\tPRICE\t\tSTOCK\n");
        			printf("%-10d\t",items[i]->item_id);
        			printf("%-15s\t",items[i]->item_name);
        			printf("%-15s\t",items[i]->item_cate);
...
void insert (void)
{
    ...
    for(i=0;i<j;i++){
        ...
        if(strcmp(name,items[i]->item_name)==0){
            printf("The item has already existed!!\n");
            items[j] -> item_id = NULL;       //clear the item id has stored jut now
            return;
        }
        strcpy(items[j]->item_name, name);
    }
